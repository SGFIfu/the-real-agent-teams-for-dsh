    ...
    React.useEffect(() => {
        ...
        const hydrate = async () => {
            ...
            if (address === undefined && leadSessionId !== undefined && sessions?.refreshSubagents !== undefined) {
                try {
                    await sessions.refreshSubagents(leadSessionId);
                }
                catch { /* address remains unavailable */ }
            ...
            if (address !== undefined && sessions?.openSubagent !== undefined) {
                ...
                catch { /* binding may still be retained */ }
            }
...
function apply(ctx) {
    ...
    const bridge = {
        async listTeams() {
            return (await readResponse(await fetch('/agent-teams/teams', { credentials: 'same-origin' })));
        },
