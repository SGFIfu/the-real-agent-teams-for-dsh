...
export function registerTeamTools(deps) {
    ...
    register(defineTool(deps, 'team_member_spawn', 'Spawn a durable continuable teammate on the native subagent runtime (lead only). The teammate receives the team protocol and keeps working across tasks until stopped.', params({
        ...
        try {
            ...
            if (args.taskId !== undefined)
                await s.assignTask(args.taskId, member.id, actor);
            ...
            try {
                runtime.interrupt(spawn.childId, leadHandle(team.leadSessionId));
            }
            catch { }
            throw error;
        }
    ...
    }, ['teamId', 'sessionId', 'name', 'role']), (args, actor, s) => s.registerMember({ teamId: args.teamId, sessionId: args.sessionId, name: args.name, role: args.role, modelProvider: args.modelProvider, model: args.model, capabilities: args.capabilities, actor })));
    ...
    register(defineTool(deps, 'team_workspace_create', 'Create a durable, session-bound Git workspace lease for implementation work.', params({
        teamId: stringProp('Team id'),
    ...
    }, ['teamId', 'repositoryRoot', 'branch', 'worktreePath']), (args, actor, s) => s.createWorkspace({ teamId: args.teamId, taskId: args.taskId, repositoryRoot: args.repositoryRoot, branch: args.branch, worktreePath: args.worktreePath }, actor)));
    register(defineTool(deps, 'team_workspace_list', 'List durable workspaces visible to the calling Team member.', params({ teamId: stringProp('Team id') }, ['teamId']), (args, actor, s) => s.listWorkspaces(args.teamId, actor)));
    register(defineTool(deps, 'team_workspace_get', 'Read one session-bound workspace lease.', params({ teamId: stringProp('Team id'), workspaceId: stringProp('Workspace id') }, ['teamId', 'workspaceId']), (args, actor, s) => s.getWorkspace(args.workspaceId, args.teamId, actor)));
    register(defineTool(deps, 'team_workspace_heartbeat', 'Renew the calling member workspace lease.', params({ teamId: stringProp('Team id'), workspaceId: stringProp('Workspace id') }, ['teamId', 'workspaceId']), (args, actor, s) => s.heartbeatWorkspace(args.workspaceId, args.teamId, actor)));
    register(defineTool(deps, 'team_workspace_release', 'Release the calling member workspace lease.', params({ teamId: stringProp('Team id'), workspaceId: stringProp('Workspace id') }, ['teamId', 'workspaceId']), (args, actor, s) => s.releaseWorkspace(args.workspaceId, args.teamId, actor)));
    // ── file claims ────────────────────────────────────────────────────────────
    ...
    }, ['teamId', 'patterns', 'purpose']), (args, actor, s) => s.claimFiles({ teamId: args.teamId, ownerSessionId: actor, workspaceId: args.workspaceId, patterns: args.patterns, purpose: args.purpose })));
    ...
    register(defineTool(deps, 'team_file_handoff', 'Hand off an owned file lease to another real Team member after coordination.', params({ teamId: stringProp('Team id'), claimId: stringProp('File claim id'), toSessionId: stringProp('Target member session id'), toMemberId: optionalStringProp('Target member id'), purpose: optionalStringProp('Updated ownership purpose') }, ['teamId', 'claimId', 'toSessionId']), (args, actor, s) => s.handoffFile({ teamId: args.teamId, claimId: args.claimId, toSessionId: args.toSessionId, toMemberId: args.toMemberId, purpose: args.purpose }, actor)));
    // ── review findings ────────────────────────────────────────────────────────
