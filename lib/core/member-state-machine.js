 ...
 */
export function resolveEffectiveStatus(currentStatus, desiredStatus, hasActiveTask) {
    // If transition is invalid, keep current status
    if (!isValidTransition(currentStatus, desiredStatus)) {
        console.warn(`[state-machine] Rejected invalid transition: ${currentStatus} -> ${desiredStatus}`);
    ...
    if (desiredStatus === 'idle' && hasActiveTask && (currentStatus === 'working' || currentStatus === 'blocked' || currentStatus === 'reviewing')) {
        console.warn(`[state-machine] Rejected idle transition while task is active (current: ${currentStatus})`);
        return currentStatus;
    }
    // Allow transition
