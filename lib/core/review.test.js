...
function evidence(reviewer = REVIEWER, outcome = 'passed') {
    return {
...
async function fixture() {
    ...
    const members = [
        { id: 'member-architect', teamId: team.id, sessionId: ARCHITECT, name: 'Architect', role: 'architect', status: 'idle', joinedAt: now, lastActiveAt: now },
        { id: 'member-backend', teamId: team.id, sessionId: BACKEND, name: 'Backend', role: 'backend', status: 'idle', joinedAt: now, lastActiveAt: now },
        { id: 'member-reviewer', teamId: team.id, sessionId: REVIEWER, name: 'Reviewer', role: 'reviewer', status: 'idle', joinedAt: now, lastActiveAt: now },
    ];
    const task = {
        id: 'task-review',
        teamId: team.id,
    ...
    for (const member of members)
        await store.put('members', member.id, member);
...
describe('review domain', () => {
    ...
    it('rejects illegal and repeated state transitions', async () => {
        const value = await makeRequest();
        await assert.rejects(() => value.domain.submitResult({ requestId: value.request.id, reviewerSessionId: REVIEWER, verdict: 'approved', evidence: [evidence()] }), (error) => {
            assert.equal(error.code, 'REVIEW_STATE_INVALID');
            return true;
        });
        await value.domain.startReview(value.request.id, REVIEWER);
        await assert.rejects(() => value.domain.startReview(value.request.id, REVIEWER), (error) => {
            assert.equal(error.code, 'REVIEW_STATE_INVALID');
            return true;
        });
    ...
    it('requires same-team actors and separates author from reviewer', async () => {
        const value = await fixture();
        await assert.rejects(() => value.domain.createRequest({ teamId: value.team.id, taskId: value.task.id, workspaceId: value.workspace.id, requestedBy: REVIEWER, reviewerMemberId: value.reviewer.id, baseRef: 'main', headRef: 'feature' }), (error) => {
            assert.equal(error.code, 'REVIEW_ACTOR_INVALID');
            return true;
        });
        await assert.rejects(() => value.domain.createRequest({ teamId: value.team.id, taskId: value.task.id, workspaceId: value.workspace.id, requestedBy: OUTSIDER, reviewerMemberId: value.reviewer.id, baseRef: 'main', headRef: 'feature' }), (error) => {
            assert.equal(error.code, 'UNAUTHORIZED_TEAM_ACCESS');
            return true;
        });
        await assert.rejects(() => value.domain.startReview('missing-review', REVIEWER), (error) => {
    ...
    it('cannot approve an unresolved medium/high finding or omit its linkage', async () => {
        ...
        await value.domain.startReview(value.request.id, REVIEWER);
        await assert.rejects(() => value.domain.submitResult({ requestId: value.request.id, reviewerSessionId: REVIEWER, verdict: 'approved', evidence: [evidence()], findingIds: [finding.id] }), (error) => {
            assert.equal(error.code, 'REVIEW_NOT_APPROVABLE');
            return true;
        });
        await assert.rejects(() => value.domain.submitResult({ requestId: value.request.id, reviewerSessionId: REVIEWER, verdict: 'approved', evidence: [evidence()] }), (error) => {
            assert.equal(error.code, 'REVIEW_NOT_APPROVABLE');
    ...
    it('validates structured QA evidence and resolution authority', async () => {
        const value = await makeRequest();
        await value.domain.startReview(value.request.id, REVIEWER);
        await assert.rejects(() => value.domain.submitResult({ requestId: value.request.id, reviewerSessionId: REVIEWER, verdict: 'approved', evidence: [] }), (error) => {
            assert.equal(error.code, 'QA_EVIDENCE_MISSING');
            return true;
        });
        await assert.rejects(() => value.domain.submitResult({ requestId: value.request.id, reviewerSessionId: REVIEWER, verdict: 'approved', evidence: [evidence(BACKEND)] }), (error) => {
            assert.equal(error.code, 'QA_EVIDENCE_INVALID');
            return true;
        });
        const finding = await value.domain.createFinding({
            teamId: value.team.id,
        ...
        await assert.rejects(() => value.domain.resolveFinding(finding.id, ARCHITECT, 'changed naming'), (error) => {
            assert.equal(error.code, 'UNAUTHORIZED_TEAM_ACCESS');
            return true;
        });
    });
