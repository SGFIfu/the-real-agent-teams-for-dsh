 ...
 */
export type TeamErrorCode = 'TEAM_NOT_FOUND' | 'TEAM_NOT_ACTIVE' | 'MEMBER_NOT_FOUND' | 'TASK_NOT_FOUND' | 'TASK_ALREADY_CLAIMED' | 'TASK_REOPEN_NOT_ALLOWED' | 'TASK_NOT_OWNED_BY_CALLER' | 'TASK_DEPENDENCIES_UNRESOLVED' | 'DEPENDENCY_CYCLE' | 'DEPENDENCY_SELF_REFERENCE' | 'UNAUTHORIZED_TEAM_ACCESS' | 'FILE_CLAIM_CONFLICT' | 'FILE_CLAIM_NOT_FOUND' | 'PLAN_REQUIRED' | 'PLAN_NOT_APPROVED' | 'PLAN_NOT_FOUND' | 'TEAM_NOT_COMPLETABLE' | 'SUBAGENT_UNAVAILABLE' | 'SUBAGENT_CAPABILITY_UNSUPPORTED' | 'STORAGE_UNAVAILABLE' | 'MESSAGE_NOT_FOUND' | 'INVALID_INPUT' | 'MEMBER_ALREADY_IN_TEAM' | 'REVIEW_FINDINGS_OPEN';
