...
import { isTeamError } from "./errors.js";
import { S } from "./testing.js";
...
describe('no-model team simulation', () => {
    it('runs a full 20-task development team to completion with all invariants', async () => {
        const events = [];
        const sink = { emit: (name, payload) => events.push({ name, payload }) };
        ...
        for (const task of backend.slice(1))
            await service.addDependency(team.id, task.id, arch.id, S.lead);
        for (const task of frontend)
            await service.addDependency(team.id, task.id, arch.id, S.lead);
        for (const task of [...backend, ...frontend])
            await service.addDependency(team.id, integration.id, task.id, S.lead);
        for (const task of tests)
            await service.addDependency(team.id, task.id, integration.id, S.lead);
        for (let i = 0; i < polish.length; i += 1)
            await service.addDependency(team.id, polish[i].id, tests[i % tests.length].id, S.lead);
        for (const task of [docs, perf])
            await service.addDependency(team.id, task.id, integration.id, S.lead);
        for (const task of [...polish, docs, perf])
            await service.addDependency(team.id, finalReview.id, task.id, S.lead);
        // ── members ──────────────────────────────────────────────────────────────
        const roles = [
        ...
        for (const role of roles)
            await service.registerMember({ teamId: team.id, sessionId: role.sessionId, name: role.name, role: role.role, actor: S.lead });
        const allDone = (snapshotTasks) => snapshotTasks.every((x) => x.status === 'completed');
        const worker = (sessionId, role, work, lane) => (async () => {
            for (let round = 0; round < 600; round += 1) {
                try {
                    const tasks = await service.listTasks(team.id, sessionId);
                    if (allDone(tasks))
                        return;
                    if (lane !== undefined && lane.laneDone(tasks))
                        return;
                    ...
                    if (isTeamError(error) && error.code === 'UNAUTHORIZED_TEAM_ACCESS')
        ...
        const reviewerWorker = worker(S.reviewer, 'reviewer', async (task) => (task.title === 'docs' ? 'docs reviewed' : `${task.title} done`), {
            inLane: (task) => task.title === 'docs',
            laneDone: (tasks) => tasks.some((t) => t.title === 'docs' && t.status === 'completed'),
        });
        const backendSecond = worker(S.backend, 'backend-2', async (task) => `${task.title} done`, {
        ...
        await assert.rejects(() => service.completeTeam(team.id, S.lead), (error) => {
