...
describe('service authority and multi-record guards', () => {
    it('keeps team controls and member registration lead-only', async () => {
        ...
        await assert.rejects(() => service.pauseTeam(teamId, S.backend), (error) => {
            assert.equal(error.code, 'UNAUTHORIZED_TEAM_ACCESS');
            return true;
        });
        await assert.rejects(() => service.registerMember({ teamId, sessionId: S.frontend, name: 'frontend', role: 'frontend', actor: S.backend }), (error) => {
            assert.equal(error.code, 'UNAUTHORIZED_TEAM_ACCESS');
            return true;
        });
    });
    ...
    it('serializes competing file claims in one service process', async () => {
        ...
        assert.equal(rejected.reason.code, 'FILE_CLAIM_CONFLICT');
    });
    it('rejects cross-team finding references and invalid plan transitions', async () => {
        ...
        await assert.rejects(() => first.service.addFinding({ teamId: first.teamId, authorSessionId: S.lead, taskId: task.id, severity: 'medium', summary: 'bad link', detail: 'cross-team' }), (error) => {
        ...
        await assert.rejects(() => first.service.approvePlan(plan.id, S.lead), (error) => {
            assert.equal(error.code, 'INVALID_INPUT');
            return true;
        });
    });
