...
import { WorkspaceManager, normalizeHostPath } from "./workspace.js";
import { makeService, S } from "./testing.js";
...
describe('workspace leases', () => {
    it('normalizes host paths and rejects escaping approved roots', async () => {
        ...
        assert.throws(() => normalizeHostPath('../../outside'), (error) => error.code === 'WORKSPACE_PATH_ESCAPE');
    ...
    it('binds a workspace to the real member session, recovers stale leases, and supports handoff', async () => {
        ...
        await assert.rejects(() => manager.get(workspace.id, { teamId, memberId: frontendId, sessionId: S.frontend }), (error) => error.code === 'WORKSPACE_OWNERSHIP_CONFLICT');
        clock += 101;
    ...
    it('serializes cross-agent file leases and supports conflict resolution by handoff', async () => {
        ...
        await assert.rejects(() => manager.claimFiles({
            teamId,
            workspaceId: workspace.id,
            actor: { teamId, memberId: frontendId, sessionId: S.frontend },
            patterns: ['shared/types.ts'],
            purpose: 'frontend types',
        }), (error) => error.code === 'WORKSPACE_OWNERSHIP_CONFLICT' || error.code === 'FILE_CLAIM_CONFLICT');
