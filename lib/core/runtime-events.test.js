...
import { createRuntimeEventLog } from "./runtime-events.js";
import { MemoryStore } from "./store.js";
const TEAM_A = 'team-a';
const TEAM_B = 'team-b';
function event(teamId, name, options = {}) {
...
describe('runtime event audit/recovery helper', () => {
    ...
    it('serializes concurrent fallback appends in one process but exposes its boundary', async () => {
        ...
        const events = await Promise.all(Array.from({ length: 32 }, (_, index) => log.append(event(TEAM_A, 'task_event', { dedupeKey: `event-${index}` }))));
        assert.equal(new Set(events.map((item) => item.sequence)).size, 32);
        assert.deepEqual(events.map((item) => item.sequence).sort((a, b) => a - b), Array.from({ length: 32 }, (_, index) => index + 1));
    });
