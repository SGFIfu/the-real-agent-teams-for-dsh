...
function runtimeWithWake(calls) {
    return {
...
describe('runtime reliability invariants', () => {
    ...
    it('does not allow a stopped member or a member with an active task to claim another task', async () => {
        ...
        // Transition member from 'starting' to 'idle'
        await service.updateMember(member.id, lead, { status: 'idle' });
        ...
        await assert.rejects(() => service.claimTask(second.id, member.sessionId), (error) => error.code === 'TASK_NOT_ELIGIBLE');
    ...
    it('releases owned work when the native worker fails', async () => {
        ...
        await assert.rejects(() => service.claimTask(task.id, 'worker'), (error) => error.code === 'TASK_NOT_ELIGIBLE');
