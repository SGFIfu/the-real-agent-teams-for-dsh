...
import { resolveEffectiveStatus, validateTransition } from "./member-state-machine.js";
...
export class AgentTeamsService {
     ...
     */
    async updateMemberVersioned(memberId, expectedVersion, updater) {
     ...
     */
    async cleanupTeamTimers(teamId) {
     ...
     */
    async cleanupMemberResources(memberId) {
        const member = await this.getMember(memberId);
        ...
        const pendingMessages = await this.store.list('messages', (message) => message.teamId === teamId &&
        ...
        const ownedClaims = await this.store.list('file_claims', (claim) => claim.teamId === teamId && claim.ownerSessionId === sessionId);
