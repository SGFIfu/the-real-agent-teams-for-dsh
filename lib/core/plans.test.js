...
describe('plans', () => {
    ...
    it('submitting a plan for a non-requiresPlan task is rejected', async () => {
        ...
        await assert.rejects(() => service.submitPlan({ teamId, taskId: task.id, authorSessionId: S.lead, body: 'x' }), (error) => {
    ...
    it('non-lead cannot approve', async () => {
        ...
        await assert.rejects(() => service.approvePlan(plan.id, S.backend), (error) => {
    ...
    it('completion guard requires an approved plan for requiresPlan tasks', async () => {
        ...
        await assert.rejects(() => service.completeTeam(teamId, S.lead), (error) => {
            ...
            assert.match(JSON.stringify(error.details ?? {}), /approved plan/);
            return true;
        });
    ...
    it('hard completion guard rejects implementation before plan approval', async () => {
        ...
        await service.claimTask(task.id, S.backend);
        await assert.rejects(() => service.completeTask(task.id, S.backend, 'premature'), (error) => {
    ...
    it('hard implementation guard rejects file ownership before plan approval', async () => {
        ...
        await service.claimTask(task.id, S.backend);
        await assert.rejects(() => service.claimFiles({ teamId, ownerSessionId: S.backend, patterns: ['backend/**'], purpose: 'implementation' }), (error) => {
            assert.equal(error.code, 'PLAN_NOT_APPROVED');
            assert.match(error.message, /before implementation/);
            return true;
        });
    });
