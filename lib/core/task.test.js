...
describe('task lifecycle', () => {
    ...
    it('completes and self-claims the next task without changing session identity', async () => {
        ...
        if (result.next.claimed)
            assert.equal(result.next.task.id, second.id);
    ...
    it('rejects a second claim of the same task', async () => {
        ...
        await assert.rejects(() => service.claimTask(task.id, S.frontend), (error) => {
    ...
    it('rejects completion by a non-owner', async () => {
        ...
        const task = await service.createTask({ teamId, title: 'x', description: 'y', actor: S.lead });
        await service.claimTask(task.id, S.backend);
        await assert.rejects(() => service.completeTask(task.id, S.frontend, 'sneaky'), (error) => {
            assert.equal(error.code, 'TASK_NOT_OWNED_BY_CALLER');
            return true;
        });
    });
    ...
    it('does not reopen a completed dependency behind active downstream work', async () => {
        ...
        await assert.rejects(() => service.reassignTask(prerequisite.id, S.lead, S.frontend), (error) => {
            assert.ok(error instanceof TeamError);
    ...
    it('rejects access from non-members', async () => {
        const { service, teamId } = await makeFixture([]);
        await assert.rejects(() => service.listTasks(teamId, S.outsider), (error) => {
    ...
    it('rejects a second member registration of the same session', async () => {
        const { service, teamId } = await makeFixture([{ name: 'a', role: 'a', sessionId: S.backend }]);
        await assert.rejects(() => service.registerMember({ teamId, sessionId: S.backend, name: 'a2', role: 'a', actor: S.lead }), (error) => {
            assert.equal(error.code, 'MEMBER_ALREADY_IN_TEAM');
            return true;
        });
    });
    it('enforces the member cap', async () => {
        ...
        await assert.rejects(() => service.registerMember({ teamId: team.id, sessionId: S.tester, name: 'c', role: 'c', actor: S.lead }), (error) => {
    ...
    it('pauses and resumes a team', async () => {
        ...
        await assert.rejects(() => service.createTask({ teamId, title: 'x', description: 'y', actor: S.lead }), (error) => {
            assert.equal(error.code, 'TEAM_NOT_ACTIVE');
            return true;
        });
