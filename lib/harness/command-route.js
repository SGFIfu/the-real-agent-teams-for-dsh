...
function authenticatedMutation(req, sessions) {
    ...
    if (csrf === undefined || csrf.length !== session.csrf.length)
        return false;
...
function safeId(raw) {
    ...
    if (decoded.includes('/') || decoded.includes('\\') || decoded.includes('..'))
        throw new Error('unsafe resource id');
    return decoded;
}
...
export function commandRoute(service, deps, sseClients) {
    ...
    return {
        ...
        handler: async (req, res) => {
            ...
            const segments = url.split('/').filter((p) => p.length > 0);
            ...
            try {
                ...
                if (method === 'GET') {
                    ...
                    if (after.length === 1 && after[0] === 'stream') {
                        ...
                        if (validSession(req, sessions) === undefined)
                            return json(res, 401, { error: 'authenticated Harness browser session required' });
                ...
                if (method === 'POST') {
                    ...
                    if (after.length === 3 && after[0] === 'team' && after[2] === 'interrupt') {
                        ...
                        if (target !== team.leadSessionId && (await service.memberBySession(teamId, target)) === undefined)
                            return json(res, 403, { error: 'target session is not a member of team' });
