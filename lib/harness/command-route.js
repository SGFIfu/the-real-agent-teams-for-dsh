...
function issueSession(req, res, sessions) {
    ...
    res.setHeader('Set-Cookie', `${SESSION_COOKIE}=${encodeURIComponent(id)}; Path=/agent-teams; Max-Age=${Math.floor(SESSION_TTL_MS / 1000)}; HttpOnly; SameSite=Strict`);
    res.setHeader('Cache-Control', 'no-store');
    res.setHeader('X-Agent-Teams-CSRF', session.csrf);
    return session;
}
function validSession(req, sessions) {
...
function loopbackAddress(address) {
    ...
    if (address.startsWith('::ffff:'))
        return loopbackAddress(address.slice('::ffff:'.length));
...
function sameOriginLoopback(req) {
    if (!loopbackAddress(req.socket.remoteAddress) || !loopbackAddress(req.socket.localAddress))
        return false;
    const host = header(req, 'host');
    if (host !== undefined && !loopbackHost(host))
        return false;
    ...
    if (origin !== undefined) {
        try {
            ...
            if (parsedOrigin.protocol !== 'http:' && parsedOrigin.protocol !== 'https:')
                return false;
            if (!loopbackHost(parsedOrigin.host) || (expectedHost !== undefined && parsedOrigin.host !== expectedHost))
                return false;
        }
        catch {
            return false;
    ...
    if (origin === undefined && referer !== undefined) {
        try {
            ...
            if (!loopbackHost(parsedReferer.host) || (host !== undefined && parsedReferer.host !== new URL(`http://${host}`).host))
                return false;
        }
        catch {
            return false;
...
function authenticatedMutation(req, sessions) {
    if (!sameOriginLoopback(req))
        throw teamError('WEB_ORIGIN_FORBIDDEN', 'web caller must be a same-origin loopback request');
...
function readBody(req) {
    return new Promise((resolve, reject) => {
        ...
        req.on('error', (error) => {
            if (!rejected)
                reject(teamError('INVALID_INPUT', `request body could not be read: ${String(error)}`));
        });
        req.on('end', () => {
            if (rejected)
                return;
...
function bodyId(body, field, required = true) {
    const value = body[field];
    if (value === undefined && !required)
        return undefined;
    if (typeof value !== 'string' || value.length === 0)
        throw teamError('INVALID_INPUT', `${field} must be a non-empty string`);
    return safeId(value);
}
function bodyText(body, field, maxLength, required = true) {
    const value = body[field];
    if (value === undefined && !required)
        return '';
    if (typeof value !== 'string')
        throw teamError('INVALID_INPUT', `${field} must be a string`);
    if (value.length > maxLength)
        throw teamError('INVALID_INPUT', `${field} exceeds the ${maxLength} character limit`);
    return value;
}
async function authorizeMutation(req, deps, authenticated, teamId, mutation) {
...
export function commandRoute(service, deps, sseClients) {
                ...
                if (method === 'POST') {
                    ...
                    if (after.length === 5 && after[0] === 'team' && after[2] === 'plan') {
                        ...
                        if (!plans.some((plan) => plan.id === planId))
                            return json(res, 403, { error: 'plan does not belong to team' });
                        if (after[4] === 'approve') {
                    ...
                    if (after.length === 3 && after[0] === 'team' && after[2] === 'interrupt') {
                        ...
                        const body = await readBody(req);
                        if (deps === undefined)
                            return json(res, 503, { error: 'subagent runtime not mounted' });
                        const team = await service.getTeam(teamId);
                        const target = bodyId(body, 'sessionId');
                    ...
                    if (after.length === 4 && after[0] === 'team' && after[2] === 'member' && after[3] === 'remove') {
                        ...
                        if (member.teamId !== teamId)
                            return json(res, 403, { error: 'member does not belong to team' });
