...
describe('provider/model resolution', () => {
    ...
    it('rejects an unregistered runtime provider before durable member creation', () => {
        assert.throws(() => resolveAgentSpec({ provider: 'missing-runtime' }, { availableProviders: ['spawn'], defaultProvider: 'spawn' }), (error) => error.code === 'SUBAGENT_PROVIDER_NOT_FOUND' && error.details?.availableProviders?.toString() === 'spawn');
    });
});
