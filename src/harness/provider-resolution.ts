...
export function resolveAgentSpec(
  ...
  options: { availableProviders: readonly string[]; defaultProvider: string; defaultModel?: string },
): ResolvedAgentSpec {
  ...
  if (!availableProviders.includes(resolvedProvider)) {
    throw teamError(
      ...
      'subagent provider ' + resolvedProvider + ' is not registered',
      {
        requestedModel,
  ...
  if (aliasEntry !== undefined && requestedProvider !== undefined && modelAlias !== undefined && requestedProvider !== aliasEntry.provider) {
    throw teamError(
      'SUBAGENT_MODEL_PROVIDER_INCOMPATIBLE',
      'model alias ' + alias + ' requires provider ' + aliasEntry.provider + ', not ' + requestedProvider,
      {
        requestedModel,
        requestedModelProvider,
        resolvedModel,
