...
import { domainSchema } from './schemas.ts';
import { MemoryStore } from './store.ts';
import { dumpStore, makeFixture, makeService, S } from './testing.ts';
...
describe('persistence', () => {
  it('every durable record survives a zod round-trip', async () => {
    ...
    for (const [table, records] of Object.entries(dump)) {
      const schema = domainSchema[table as keyof typeof domainSchema];
...
describe('completion guard and snapshot', () => {
  it('rejects completion with incomplete required tasks and open critical findings', async () => {
    ...
    await assert.rejects(() => service.completeTeam(teamId, S.lead), (error: unknown) => {
      ...
      const reasons = ((error as TeamError).details?.reasons as string[]).join(' ');
  ...
  it('critical blocked tasks block completion', async () => {
    ...
    await assert.rejects(() => service.completeTeam(teamId, S.lead), (error: unknown) => {
      assert.equal((error as TeamError).code, 'TEAM_NOT_COMPLETABLE');
      assert.match(JSON.stringify((error as TeamError).details ?? {}), /critical tasks blocked/);
      return true;
    });
  });
  ...
  it('lead-only completion and actor restrictions hold', async () => {
    const { service, teamId } = await makeFixture([{ name: 'b', role: 'b', sessionId: S.backend }]);
    await assert.rejects(() => service.completeTeam(teamId, S.backend), (error: unknown) => {
