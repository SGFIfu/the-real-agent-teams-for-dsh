...
describe('task dependencies', () => {
  it('A → B: B cannot be claimed before A completes', async () => {
    ...
    await assert.rejects(() => service.claimTask(b.id, S.backend), (error: unknown) => {
      assert.equal((error as TeamError).code, 'TASK_DEPENDENCIES_UNRESOLVED');
      return true;
    });
  ...
  it('rejects the cycle A → B → C → A', async () => {
    ...
    await assert.rejects(() => service.addDependency(teamId, c.id, a.id, S.lead), (error: unknown) => {
      assert.equal((error as TeamError).code, 'DEPENDENCY_CYCLE');
      return true;
    });
  });
  ...
  it('rejects the 2-cycle A → B, B → A', async () => {
    ...
    const b = await service.createTask({ teamId, title: 'B', description: 'b', actor: S.lead });
    await service.addDependency(teamId, a.id, b.id, S.lead);
    await assert.rejects(() => service.addDependency(teamId, b.id, a.id, S.lead), (error: unknown) => {
      assert.equal((error as TeamError).code, 'DEPENDENCY_CYCLE');
      return true;
    });
  });
  ...
  it('rejects self-dependency', async () => {
    const { service, teamId } = await makeFixture([]);
    const a = await service.createTask({ teamId, title: 'A', description: 'a', actor: S.lead });
    await assert.rejects(() => service.addDependency(teamId, a.id, a.id, S.lead), (error: unknown) => {
      assert.equal((error as TeamError).code, 'DEPENDENCY_SELF_REFERENCE');
      return true;
    });
  });
  ...
  it('diamond DAG: integration task waits for both branches', async () => {
    ...
    await assert.rejects(() => service.claimTask(integration.id, S.backend), (error: unknown) => {
      assert.equal((error as TeamError).code, 'TASK_DEPENDENCIES_UNRESOLVED');
      return true;
    });

    await service.completeTask(backend.id, S.backend, 'ok');
    await assert.rejects(() => service.claimTask(integration.id, S.frontend), (error: unknown) => {
      assert.equal((error as TeamError).code, 'TASK_DEPENDENCIES_UNRESOLVED');
      return true;
    });
  ...
  it('cross-team dependency is rejected', async () => {
    ...
    await assert.rejects(() => service.addDependency(teamId, a.id, b.id, S.lead), (error: unknown) => {
