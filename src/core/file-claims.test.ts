...
describe('file claims', () => {
  ...
  it('exact path conflict between owners is rejected', async () => {
    ...
    await assert.rejects(
      () => service.claimFiles({ teamId, ownerSessionId: S.frontend, patterns: ['src/server/auth.ts'], purpose: 'frontend' }),
      (error: unknown) => {
  ...
  it('directory vs contained file conflicts', async () => {
    ...
    await assert.rejects(
      () => service.claimFiles({ teamId, ownerSessionId: S.frontend, patterns: ['src/server/auth.ts'], purpose: 'one file' }),
      (error: unknown) => {
        assert.equal((error as TeamError).code, 'FILE_CLAIM_CONFLICT');
        return true;
      },
  ...
  it('glob vs contained path conflicts; disjoint globs do not', async () => {
    ...
    await assert.rejects(
      () => service.claimFiles({ teamId, ownerSessionId: S.frontend, patterns: ['src/server/auth.ts'], purpose: 'auth' }),
      (error: unknown) => {
        assert.equal((error as TeamError).code, 'FILE_CLAIM_CONFLICT');
        return true;
      },
  ...
  it('atomic batch: no partial claims when one pattern conflicts', async () => {
    ...
    await service.claimFiles({ teamId, ownerSessionId: S.backend, patterns: ['src/server/auth.ts'], purpose: 'backend' });
    await assert.rejects(
      () => service.claimFiles({ teamId, ownerSessionId: S.frontend, patterns: ['src/client/app.ts', 'src/server/auth.ts'], purpose: 'mixed' }),
      (error: unknown) => {
        assert.equal((error as TeamError).code, 'FILE_CLAIM_CONFLICT');
        return true;
      },
  ...
  it('non-owner cannot release; lead can', async () => {
    ...
    await assert.rejects(() => service.releaseFiles([claims[0].id], S.frontend), (error: unknown) => {
