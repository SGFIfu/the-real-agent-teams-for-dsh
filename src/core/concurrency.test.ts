...
describe('atomic task claiming', () => {
  it('50 concurrent claims on ONE task yield exactly one owner', async () => {
    ...
    const wins = results.filter((r) => r.status === 'fulfilled').map((r) => (r as PromiseFulfilledResult<TeamTask>).value);
  ...
  it('4 agents self-scheduling over 20 tasks: no duplicate owner, all claimed once', async () => {
    ...
    const perAgent: TeamTask[][] = await Promise.all(
    ...
    const owners = claimed.map((t) => t.ownerSessionId as string);
    ...
    for (const task of fresh) assert.equal(task.status, 'in_progress');
    for (const task of fresh) assert.ok(task.ownerSessionId !== undefined);
    const ownershipCount = new Map<string, number>();
    for (const task of fresh) ownershipCount.set(task.ownerSessionId as string, (ownershipCount.get(task.ownerSessionId as string) ?? 0) + 1);
  ...
  it('50-way parallel claimNext: exactly one owner per task, 20 tasks claimed once', async () => {
    ...
    const wins = rounds.filter((r) => r.claimed).map((r) => (r as { claimed: true; task: TeamTask }).task);
  ...
  it('dependency-respecting claiming under concurrency: dependent tasks never claimed first', async () => {
    ...
    assert.equal((first as { claimed: true; task: TeamTask }).task.id, a.id, 'root task claimed before dependent');
    ...
    assert.equal((third as { claimed: true; task: TeamTask }).task.id, b.id);
  });
});
