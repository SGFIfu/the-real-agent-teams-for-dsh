...
import { domainSchema, findingSchema, messageSchema } from './schemas.ts';
import { MemoryStore, type TableName } from './store.ts';
...
describe('shared domain contracts', () => {
  ...
  it('initializes all new MemoryStore tables and supports typed records', async () => {
    ...
    for (const table of newTables) assert.deepEqual(await store.list(table), [], `${table} starts empty`);

    await store.put('runtime_events', 'event-1', {
      id: 'event-1' as RuntimeEvent['id'],
      teamId: 'team-1' as RuntimeEvent['teamId'],
      sequence: 1,
  ...
  it('accepts a complete record for each new contract schema', () => {
    ...
    for (const table of newTables) domainSchema[table].parse(records[table]);
  });
