...
test('pushBuffer is bounded, sorted and deduplicated', () => {
  const events: UiEvent[] = Array.from({ length: 20 }, (_, i) => ({ id: `e${i}`, kind: 'message', ts: i, teamId: 't', title: `m${i}` }));
...
test('normalizeSnapshot coerces the raw payload into the UI model', () => {
  const raw = {
    team: { id: 't', name: 'X', goal: 'Ship', createdAt: 123, status: 'active' },
    members: [{ id: 'm1', sessionId: 's1', name: 'A', role: 'lead', status: 'idle' }],
    tasks: [{ id: 't1', title: 'W', description: 'Explain the work', status: 'pending', priority: 'high', ownerSessionId: undefined, dependencies: ['t0'], result: undefined }],
    plans: [],
    findings: [],
    messages: [],
  ...
  assert.equal(ui.teamGoal, 'Ship');
  assert.equal(ui.teamCreatedAt, 123);
  assert.equal(ui.teamId, 't');
  assert.equal(ui.members[0].name, 'A');
  assert.equal(ui.tasks[0].description, 'Explain the work');
