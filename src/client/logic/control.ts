...
export interface UiSnapshot {
  ...
  messages: UiMessage[];
  fileClaims: UiClaim[];
  progress: { requiredTotal: number; requiredDone: number; ratio: number; blocked: string[]; awaitingPlanApproval: string[] };
}
...
export function statusMeta(status: string): { icon: string; label: string; css: string } {
  switch (status) {
    case 'working':
...
export function taskStatusMeta(status: string): { icon: string; label: string; css: string } {
  switch (status) {
    case 'pending':
...
export function layeredGraph(tasks: UiTask[]): UiTask[][] {
  const rows: UiTask[][] = [];
  ...
  while (remaining.length > 0 && guard < 1000) {
    ...
    rows.push(row);
    for (const t of row) placed.add(t.id);
...
export function diffSnapshots(prev: UiSnapshot | undefined, next: UiSnapshot, nowTs: number): UiEvent[] {
  ...
  if (prev === undefined) {
    ...
    for (const t of next.tasks) {
      if (t.status === 'completed') events.push({ id: `t-${t.id}-init`, kind: 'task-completed', ts: nowTs, teamId: next.teamId, taskId: t.id, title: `Task ${t.title} completed` });
    }
    return events;
  }
 ...
 */
export function rawEventToUiEvent(frame: any, nowTs: number): UiEvent | undefined {
  if (frame === null || typeof frame !== 'object' || typeof frame.type !== 'string') return undefined;
  ...
  switch (frame.type) {
    case 'agent-teams/member-joined': {
      const m = p.member;
      if (m === undefined || typeof m !== 'object') return undefined;
    ...
    case 'agent-teams/message-sent': {
      const m = p.message;
      if (m === undefined || typeof m !== 'object') return undefined;
      if (m.deliveryState === 'failed') return { id: `msg-${m.id}-failed`, kind: 'message', ts: nowTs, teamId: teamId || m.teamId, sessionId: m.fromSessionId, targetSessionId: m.toSessionId, preview: `⚠ delivery failed: ${String(m.deliveryError ?? '')}`, title: 'Message delivery failed' };
    ...
    case 'agent-teams/plan-submitted': {
      const pl = p.plan;
      if (pl === undefined || typeof pl !== 'object') return undefined;
    ...
    case 'agent-teams/file-claimed': {
      const c = p.claim;
      if (c === undefined || typeof c !== 'object') return undefined;
...
export function pushBuffer(buffer: BufferedActivity[], events: UiEvent[], cap = 300): BufferedActivity[] {
  const merged: BufferedActivity[] = [...events.map((e) => ({ ...e, title: e.title ?? '' })), ...buffer];
  merged.sort((a, b) => b.ts - a.ts || (a.id < b.id ? 1 : -1));
  const seen = new Set<string>();
  const out: BufferedActivity[] = [];
...
export function filterActivity(activity: BufferedActivity[], filter: ActivityFilter): BufferedActivity[] {
  if (filter === 'ALL') return activity;
  const kindSet: Record<Exclude<ActivityFilter, 'ALL'>, string[]> = {
...
export function statusCounts(members: UiMember[]): Record<string, number> {
...
export function resolveSelectedTeamId(teams: readonly { id: string }[], requested: string | null | undefined): string | null {
...
export function teamIdFromHash(hash: string): string | null {
  if (!hash.startsWith('#agent-team=')) return null;
  try {
...
export function normalizeSnapshot(raw: any): UiSnapshot {
  return {
    teamId: raw.team.id,
    ...
    fileClaims: (raw.fileClaims ?? []).map((c: any) => ({ id: c.id, ownerSessionId: c.ownerSessionId, pattern: c.pattern, kind: c.kind })),
